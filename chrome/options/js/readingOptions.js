function testTts(tts, callback) {
	chrome.runtime.sendMessage({action: "clicknspeech.testTtsServices", tts:tts}, function(success) {
...
.controller('readingOptionsController', function($scope) {
	...
	chrome.runtime.sendMessage({action: "clicknspeech.getTtsProperties"}, function(ttsProperties) {
		ttsProperties.forEach(function(tts) {
			...
			chrome.runtime.sendMessage({action: "clicknspeech.testTtsService", tts:tts.name}, function(success) {
