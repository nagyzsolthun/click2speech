import * as textSplitter from "./TextSplitter.js";

const DONOTHING = () => {};

/** handle text not fitting in a single http speech request */
function HttpSpeech({ text, buildUrl, testLength, speed }) {

    // text split based on testLength
    var textArr = [];

    // audios for each string in textArr
    var audioArr = [];

    // store audio event listeners for cleanUp
    const indexToEventToListeners = {};

    // called when speech starts
    var startCallback = DONOTHING;

    // called when speech reaches sentence start
    var sentenceCallback = DONOTHING;

    // called when speech ends
    var endCallback = DONOTHING;

    // called when error
    var errorCallback = DONOTHING;

    this.play = function() {
        if(audioArr.length) audioArr[0].play();   // check if audiArr is created (aka text is not empty)
        else endCallback();
        return this;
    }

    this.stop = function() {
        cleanUp();
        endCallback();
    }

    this.onStart = function(callback) {
        startCallback = () => callback();
        return this;
    }

    this.onSentence = function(callback) {
        sentenceCallback = (startIndex) => callback(startIndex);
        return this;
    }

    this.onEnd = function(callback) {
        endCallback = () => callback();
        return this;
    }

    this.onError = function(callback) {
        errorCallback = (startIndex) => callback(startIndex);
        return this;
    }

    init();

    function init() {
        if(!text) return;   // TODO
        textArr = textSplitter.split(text,testLength);
        prepareAudio(0);
    }

    // send HttpRequests and add event listeners
    function prepareAudio(audioIndex) {
        createAudio(audioIndex);
        scheduleStart(audioIndex);

        scheduleStartCallback(audioIndex);
        scheduleSentenceStartCallback(audioIndex);
        schedulePrepareNext(audioIndex);
        scheduleCleanPlayingCallback(audioIndex);   // set after all other playing listeners set

        scheduleEndCallback(audioIndex);
        scheduleErrorCallback(audioIndex);
    }

    function createAudio(audioIndex) {
        const audio = new Audio();
        audio.src = buildUrl(textArr[audioIndex]);
        audio.playbackRate = speed;
        audioArr.push(audio);
    }

    function scheduleStart(audioIndex) {
        if(!isFirstAudio(audioIndex)) {
            addAudioEventListener(audioIndex-1, "ended", () => audioArr[audioIndex].play());
        }
    }

    function scheduleStartCallback(audioIndex) {
        if(isFirstAudio(audioIndex)) {
            addAudioEventListener(audioIndex, "playing", () => startCallback());
        }
    }

    function scheduleSentenceStartCallback(audioIndex) {
        if(isSentenceStart(audioIndex)) {
            addAudioEventListener(audioIndex, "playing", () => sentenceCallback(startIndex(audioIndex)) );
        }
    }

    function schedulePrepareNext(audioIndex) {
        if(!isLastAudio(audioIndex)) {
            addAudioEventListener(audioIndex, "playing", () => prepareAudio(audioIndex+1));    // TODO loading time?
        }
    }

    // multiple playing events can be received (due to audio waiting)
    function scheduleCleanPlayingCallback(audioIndex) {
        addAudioEventListener(audioIndex, "playing", () => removeAudioEventListeners(audioIndex, "playing") );
    }

    function scheduleEndCallback(audioIndex) {
        if(isLastAudio(audioIndex)) {
            addAudioEventListener(audioIndex, "ended", () => {
                cleanUp();
                endCallback();
            });
        }
    }

    function scheduleErrorCallback(audioIndex) {
        addAudioEventListener(audioIndex, "error", () => {
            cleanUp();
            errorCallback(startIndex(audioIndex));
        });
    }

    // add listener to audio and to internal collection for cleanUp
    function addAudioEventListener(audioIndex, event, listener) {

        // add listener to audio
        audioArr[audioIndex].addEventListener(event, listener);

        // add to internal map
        var eventToListeners = indexToEventToListeners[audioIndex];
        if(!eventToListeners) {
            eventToListeners = {};
            indexToEventToListeners[audioIndex] = eventToListeners;
        }
        var listeners = eventToListeners[event];
        if(!listeners) {
            listeners = [];
            eventToListeners[event] = listeners;
        }
        listeners.push(listener);
    }

    function removeAudioEventListeners(audioIndex, event) {
        var eventToListeners = indexToEventToListeners[audioIndex];
        var listeners = eventToListeners[event];
        if(listeners) {
            listeners.forEach(listener => audioArr[audioIndex].removeEventListener(event,listener));
            delete indexToEventToListeners[audioIndex];
        }
    }

    function isSentenceStart(audioIndex) {
        if(audioIndex == 0) {
            return true;
        }
        var previousText = textArr[audioIndex-1];
        return textSplitter.isSentence(previousText);
    }

    function startIndex(audioIndex) {
        return textArr
            .filter((text,index) => index < audioIndex)     // find texts before audioArr
            .reduce((sum,text) => sum += text.length, 0);     // sum their lenghts
    }

    function isLastAudio(audioIndex) {
        return audioIndex == textArr.length - 1;
    }

    function isFirstAudio(audioIndex) {
        return audioIndex == 0;
    }

    function cleanUp() {
        audioArr.forEach(clean);
        audioArr = [];
    }

    function clean(audio,index) {
        const eventToListeners = indexToEventToListeners[index];
        for(var event in eventToListeners) {
            const listeners = eventToListeners[event];
            listeners.forEach(listener => audio.removeEventListener(event,listener));
        }
        audio.pause();   // audio.src = "" => errorCallback when ended
    }
}

function buildHttpSpeech({ text, buildUrl, testLength, speed }) {
    return new HttpSpeech({ text, buildUrl, testLength, speed });
}

export { buildHttpSpeech };
